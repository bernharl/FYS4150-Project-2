...
The eigenvalue solver we will show is using the Jacobi algorithm, which converges quite stabely yet very slowly. The essence of the algorithm is to find a diagonal matrix $D$ similar to $A$ using several similarity transforms rotating the column space of $A$ in $n$ dimensional space until $A$ is diagonalised. Because $D$ and $A$ are similar, the diagonal elements $d_ii = \lambda_i$ being the eigenvalues of $D$ are eigenvalues of $A$ too.
